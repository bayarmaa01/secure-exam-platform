...
// Student: Get available exams (REQUIREMENT SPECIFIC QUERY)
router.get('/student/exams', auth, requireStudent, async (req: AuthRequest, res) => {
  try {
    ...
    const enrollmentCheck = await pool.query(
      'SELECT en.course_id, c.name as course_name FROM enrollments en JOIN courses c ON c.id = en.course_id WHERE en.student_id = $1',
      [studentId]
    )
    console.log('Student enrollments:', JSON.stringify(enrollmentCheck.rows, null, 2))
    
    // EXAM-COURSE RELATION DEBUG
    const examCourseCheck = await pool.query(
      'SELECT e.id, e.title, e.course_id, c.name as course_name FROM exams e JOIN courses c ON c.id = e.course_id',
      []
    )
    console.log('All exam-course relations:', JSON.stringify(examCourseCheck.rows, null, 2))

    const mappedResults = result.rows.map(exam => ({
      id: exam.id,
      title: exam.title,
      description: exam.description || '',
